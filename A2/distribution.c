...
int main(int argc, char *argv[]){
	...
	if (column_id == 1) {
		...
		max_degree = compute_max_in_degree(filename, blocksize);	
	}
	distribution(filename, blocksize, column_id, max_degree);
	return 0;
}
	...
	while((result = fread (buffer, sizeof(Record), records_per_block, fp_read)) > 0){
		...
		while(i < records_per_block){
			...
			if (current_id == -1){
				...
				current_followers = 1;
				unique_users = 1;int max_degree = atoi(argv[4]);
...
void distribution(char * filename, int block_size, char * column_id, int max_degree) {
	int records_per_block = block_size/sizeof(Record);
	FILE *fp_read;
	// allocate array 
   ...
   int flag = 1;
	...
	for (i = 1; i < max_degree; i++) {
		if (column_id == 1) {
			printf("out degree of %d has count %d", i, degree_count[i]);
		} else {
			printf("in degree of %d has count %d", i, degree_count[i]);
		}
